...
7. Fetch the selected packs with repeated `greprules fetch --pack <slug>` arguments. If no available pack fits the target, report that pack selection needs user input instead of running a broad fetch.
8. If OpenGrep is still not ready, stop and summarize `recommendedCommands`.
9. Run `greprules scan --target <path>` for each requested path.
