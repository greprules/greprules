...
- CLI config owns registry/OpenGrep runtime. Codex settings own hook behavior. Never store hooks in `greprules config set agent.*`.
...
2. Run `greprules doctor --format json`, then read Codex settings or use defaults.
...
5. For registry or `opengrep.includeDefaultRules`, use `greprules config set <key> <value> --global` unless the user asked for repo-local config.
6. For hook settings, update only the Codex settings JSON, then rerun doctor.
...
8. Apply runtime with `greprules config set ... --global`; for managed mode, also run `greprules setup-opengrep`.
9. Rerun doctor and summarize registry, OpenGrep, rule-pack state, and Codex settings.
