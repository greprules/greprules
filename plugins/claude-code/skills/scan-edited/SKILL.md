...
5. If pack selection is needed, run the exact `greprules recommend --format json --agent --targets-from <targetsPath>` command shown, inspect `detection`, `targets`, `availablePacks`, and `candidates`, then fetch only explicit matching slugs with the shown `greprules fetch --pack <slug>`. Do not invent pack slugs.
...
- If multiple Claude sessions have dirty files and no current session id is available, do not merge them; ask for a session-scoped scan or use `/greprules:scan-target <path>`.
- If edited-file tracking was disabled in Claude Code greprules settings, explain that `/greprules:scan-edited` has no source of targets.
