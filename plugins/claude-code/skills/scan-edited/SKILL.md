...
5. If the scan returns `needs_pack_selection`, inspect the returned `selectionContext.detection`, `selectionContext.targets`, `selectionContext.availablePacks`, and `selectionContext.candidates`, then fetch only explicit matching slugs with the shown `greprules fetch <slug>`. Do not invent pack slugs.
